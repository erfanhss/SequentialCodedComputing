...
def master():
    ...
    while 1:
        ...
        size_req = []
        for worker_idx in range(num_workers):
            shapeA = np.shape(coded_A[worker_idx % kA])
            shapeB = np.shape(coded_B[worker_idx // kA])
    ...
    np.savetxt(
        'train_acc_per_iteration_uncoded_fritch_h1_' + str(h1) + '_h2_' + str(
            h2) + '_epochs_' + str(
            num_epochs) + '.txt',
        np.array(train_acc))
    np.savetxt(
        'train_loss_per_iteration_uncoded_fritch_h1_' + str(h1) + '_h2_' + str(
            h2) + '_epochs_' + str(
            num_epochs) + '.txt',
        np.array(train_loss))
    np.savetxt(
        'test_acc_per_iteration_uncoded_fritch_h1_' + str(h1) + '_h2_' + str(h2) + '_epochs_' + str(
            num_epochs) + '.txt',
        np.array(test_acc))
    np.savetxt(
        'test_loss_per_iteration_uncoded_fritch_h1_' + str(h1) + '_h2_' + str(h2) + '_epochs_' + str(
            num_epochs) + '.txt',
        np.array(test_loss))
    np.savetxt(
        'result_uncoded_firtch_h1_' + str(h1) + '_h2_' + str(h2) + '_epochs_' + str(num_epochs),
        np.array(result))
    np.savetxt(
        'processing_time_uncoded_fritch_h1_' + str(h1) + '_h2_' + str(h2) + '_epochs_' + str(
            num_epochs),
        processing_times[:, 0:slot])
    np.savetxt(
        'load_uncoded_fritch_h1_' + str(h1) + '_h2_' + str(h2) + '_epochs_' + str(
            num_epochs),
