...
def master():
    ...
    while 1:
        ...
        for worker_idx in range(num_workers):
            ...
            for job_idx in range(len(queue)):
                ...
                if job['time_in_queue'] > 0 and straggling_history[worker_idx, slot-1] == 1:
                    ...
                    seen = np.sum(straggling_history[:, slot-1])
                    if T + 1 - job['time_in_queue'] > N - seen:
        ...
        for idx in range(len(times_sorted_idx)):
            ...
            if straggling_history[worker_idx, slot] == 1:
                window = straggling_history[:, slot-T:slot+1]
                if np.sum(window) < N:
                    break
                else:
                    for job_idx in range(len(queue)):
    ...
    np.savetxt(
        'train_acc_per_iteration_Arbitrary_' + str(erasure_prob) + '_h1_' + str(h1) + '_h2_' + str(h2) + '_epochs_' + str(
            num_epochs) + '.txt',
        np.array(train_acc))
    np.savetxt(
        'train_loss_per_iteration_Arbitrary_' + str(erasure_prob) + '_h1_' + str(h1) + '_h2_' + str(
            h2) + '_epochs_' + str(
            num_epochs) + '.txt',
        np.array(train_loss))
    np.savetxt(
        'test_acc_per_iteration_Arbitrary_' + str(erasure_prob) + '_h1_' + str(h1) + '_h2_' + str(h2) + '_epochs_' + str(
            num_epochs) + '.txt',
        np.array(test_acc))
    np.savetxt(
        'test_loss_per_iteration_Arbitrary_' + str(erasure_prob) + '_h1_' + str(h1) + '_h2_' + str(
            h2) + '_epochs_' + str(
            num_epochs) + '.txt',
        np.array(test_loss))

    np.savetxt(
        'result_Arbitrary_' + str(erasure_prob) + '_h1_' + str(h1) + '_h2_' + str(h2) + '_epochs_' + str(num_epochs) + '.txt',
        np.array(result))
    np.savetxt('straggling_history_Arbitrary_' + str(erasure_prob) + '_h1_' + str(h1) + '_h2_' + str(h2) + '_epochs_' + str(
        num_epochs) + '.txt',
               straggling_history[:, 0:slot])
    np.savetxt('encoding_time_Arbitrary_p_' + str(erasure_prob) + '_h1_' + str(h1) + '_h2_' + str(h2) + '_epochs_' + str(
        num_epochs) + '.txt',
               np.array(encoding_times))
    np.savetxt('decoding_time_Arbitrary_p_' + str(erasure_prob) + '_h1_' + str(h1) + '_h2_' + str(h2) + '_epochs_' + str(
        num_epochs) + '.txt',
               np.array(decoding_times))
    np.savetxt('processing_time_Arbitrary_p_' + str(erasure_prob) + '_h1_' + str(h1) + '_h2_' + str(h2) + '_epochs_' + str(
        num_epochs) + '.txt',
               processing_times[:, 0:slot])
    np.savetxt(
        'load_Arbitrary_p_' + str(erasure_prob) + '_h1_' + str(h1) + '_h2_' + str(h2) + '_epochs_' + str(
            num_epochs) + '.txt',
